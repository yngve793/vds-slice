...
TEST_F(AttributeHilbertTest, Phase_Index_K_Slice) {
    ...
    SingleDataHandle penobscot = make_single_datahandle(
...
TEST_F(AttributeHilbertTest, Phase_Attribute_2) {
    ...
    const MetadataHandle* metadata = &(penobscot.get_metadata());
    ...
    SurfaceBoundedSubVolume* subvolume = make_subvolume(penobscot.get_metadata(), pri_surface, top_surface, bot_surface);

    cppapi::fetch_subvolume(penobscot, *subvolume, NEAREST, 0, nrows * ncols);

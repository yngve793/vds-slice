...
class AttributeHilbertTest : public ::testing::Test {
    ...
    void SetUp() override {
        ...
        penobscot = make_single_datasource(
...
TEST_F(AttributeHilbertTest, Phase_Attribute_2) {
    ...
    DataSource* datasource = penobscot;
    ...
    for (int i = 0; i < nrows; i++){
        EXPECT_EQ(values[i*2], values[i*2+1]);
        EXPECT_EQ(values[i*2], value_expected[i]);
    }
    ...
    for (int i = 0; i < nrows; i++){
        ...
        EXPECT_NEAR(phases[i*2], phase_expected[i], 2e-5);
    }
    ...
    for (int i = 0; i < nrows; i++){
        ...
        EXPECT_NEAR(envelope[i*2], envelope_expected[i], 0.002);
    }
    ...
    for (int i = 0; i < nrows; i++){
        EXPECT_EQ(hilbert[i*2], hilbert[i*2+1]);
        EXPECT_NEAR(hilbert[i*2], hilbert_expected[i],  0.002);
    }

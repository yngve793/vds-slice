...
def create_10_samples(path, ilines=[1, 3, 5], xlines=[10, 11], samples=[4, 8, 12, 16, 20, 24, 28, 32, 36, 40], 
                      factor=1, samples_offset=0, ilines_offset=0, xlines_offset=0):
    ...
    spec.sorting = 2
    spec.format = 1
    spec.samples = np.array(samples) + samples_offset
    spec.ilines = np.array(ilines) + ilines_offset
    spec.xlines = np.array(xlines) + xlines_offset
            ...
            for xl in spec.xlines:
                f.header[tr] = {
                    ...
                    segyio.su.cdpy:
                        ...
                        ori_y,
                    segyio.su.scalco: -10,
                    segyio.su.delrt: 4+ samples_offset,
                }
...
if __name__ == "__main__":
    parameters = [
        {"path": "10_double_value", "factor": 2},
        {"path": "10_offset_value", "samples_offset":20, "ilines_offset":2, "xlines_offset":1},
        {"path": "10_no_intersection", "xlines_offset":2},
    ]
    for p in parameters:
        create_10_samples(**p)
        name = p["path"]

...
void attributes(
    ...
    for (int i = 0; i < nattributes; ++i) {
        ...
        switch (*attributes) {
            ...
            case ENVELOPE:  { append(attrs,   Envelope(dst, size)  );   break; }
            case HILBERT:   { append(attrs,   Hilbert(dst, size)   );   break; }
            case FREQUENCY: { append(attrs,   Frequency(dst, size) );   break; }
            case BANDWIDTH: { append(attrs,   Bandwidth(dst, size) );   break; }
            case SWEETNESS: { append(attrs,   Sweetness(dst, size) );   break; }

...
double max_abs(
    ...
    auto max = *std::max_element(segment.begin(), segment.end(), [](const double& a, const double& b) {
...
float MaxAbsAt::compute(
    ...
    auto max_abs_index = std::distance(
        segment.begin(),
        std::find_if(segment.begin(), segment.end(), [&](const double& val) {
...
float MeanAbs::compute(
    ...
    double sum = std::accumulate(segment.begin(), segment.end(), 0.0, [](double acc, double x) { return acc + std::abs(x); });
    return sum / segment.size();
}
...
float MeanPos::compute(
    ...
    int count = 0;
    double sum = std::accumulate(segment.begin(), segment.end(), 0.0, [&](double acc, double x) {
        if (x > 0) {
            count++;
            return acc + x;
        }
...
float Rms::compute(
    ...
    float sum = std::accumulate(segment.begin(), segment.end(), 0.0, [](double a, double b) {
...
double variance(
    ...
    double stdSum = std::accumulate(segment.begin(), segment.end(), 0.0, [&](double a, double b) { return a + std::pow(b - mean, 2); });
...
float Phase::compute(
    ...
    double reverence_hilbert = std::arg(result[segment.reference_index()]);

    std::cout << "Index: " << segment.reference_index() << std::endl;
    std::cout << "Reference hilbert: " << reverence_hilbert << std::endl;

    return reverence_hilbert;
}
